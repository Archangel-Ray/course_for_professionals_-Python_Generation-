"""
Каждый день Тимур записывает в блокнот дела, которые ему нужно выполнить.
    Каждое дело он разбивает на несколько действий.
...
tasks.sort()
sorted_tasks = groupby(tasks, key=lambda x: x[0])
for key, value in sorted_tasks:
    print(key, end=':\n')
    for affairs in sorted(value, key=lambda x: x[2]):
        print(f"    {affairs[2]}. {affairs[1]}")
    print()
